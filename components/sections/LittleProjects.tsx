  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        const seedOnly: LittleProject[] = seedProjects.map((p: any) => ({
          slug: p.slug,
          title: p.title,
          mainImage: p.mainImage,
          featured: Boolean((p as any).featured),
                ...
                <div className="relative group">
                  ...
                  {project.featured && (
                    <div className="absolute top-2 right-4 z-30">
                      <p className="text-sm text-[#c58d12] font-medium">
                        {texts.featuredBadge}
                      </p>
                    </div>
                  )}

                  {/* Projekt Nummer (top-left) */}
                  <div className="absolute top-4 left-4 z-30">
                    <p className="text-white font-bold text-2xl tracking-wide drop-shadow-md">
